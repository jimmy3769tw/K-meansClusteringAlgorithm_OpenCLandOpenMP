...
void distanceK(__global const float *pts,__global const float *dev_currentCentroids, uint N, uint K,uint DIM, __global float *dev_distance){
	...
	if ( i < K && whichPt < N){
		...
		for (uint whichDim = 0 ; whichDim < DIM ; whichDim ++){
			distance_sum_DIM += pow(dev_currentCentroids[i+whichDim*K]- pts[whichPt+N*whichDim],2); //*SOA
		}
...
void sumCentroidB( __global const int* group, __global float *centroids,__global float *pts, uint N, uint K, uint DIM){
	...
	// centroids [ i+whichDim*K ] += pts[whichPt + N * whichDim];
	myAtomicAddG(centroids +i + whichDim*K, pts[whichPt + N * whichDim]); //*SOA
}
...
void sumCentroidB1( __global const int* group, __global float *centroids,__global float *pts, uint N, uint K, uint DIM){
	...
	if (whichPt < N && whichDim < DIM){
		myAtomicAddL(center_ +i, pts[whichPt+N*whichDim]);// *soa
	}
	
	// * Global
	barrier(CLK_LOCAL_MEM_FENCE);
	if (get_local_id(0) ==0){
		myAtomicAddG(centroids +i + whichDim*K, center_[0][0][i]);
	}
...
void InitializeCentroid(myData& data, size_t noClusters) {
	...
	for (size_t i = 0; i < data.K; i++){
		...
		for (size_t whichDim = 0; whichDim < data.DIM; whichDim++){
			data.currentCentroids[i+whichDim*data.K] = data.pts.at(whichPt+data.N*whichDim); // *soa
		}
	}
...
void AssignGroups(myData& data) {
	...
	static cl::Buffer dev_distance(data.ctx, CL_MEM_READ_WRITE, data.N * data.K*sizeof(float));
	...
	static cl::KernelFunctor<const cl::Buffer&, cl::Buffer&, cl_uint, cl_uint> assingGroupKernel(data.prg, "assignGroup");
	...
	// 			distance_sum_DIM += pow((data.currentCentroids.at(count_k+whichDim*data.K)- data.pts.at(whichPt+data.N*whichDim),2); //*soa
	// 		}
